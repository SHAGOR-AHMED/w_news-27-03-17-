...
class User extends Base_Controller
{
    ...
    {
        $current_withdrawal_point_value = $this->user_model->getInPointForWithdraw($this->user_id);
        ...
        if ($isSuccess) {
            $this->insertToCompanyBalance(($balance->amount * Vat_Percentage) / 100, $balance->user_id, Company_Vat);
    ...
    {
        $this->load->helper('user_helper');
        $total = $this->user_model->count_user_news($this->user_id);
        $perPage = 20;
        $this->pagination(base_url('user/newsListByUser/'), $perPage, $total);
        $data['message'] = $this->user_model->get_user_receive_message($perPage, $this->uri->segment(3), $this->user_id);
        $data['content'] = $this->load->view('frontend/add_edit_news', $data, true);
        $this->load->view('frontend/index', $data);
    }
    /*
     *
...
//-------------------- News panel --------------------------------//
    ...
    {
        $data = array();
        $data['news_info'] = $this->backend_model->select_news_info();
        $data['content'] = $this->load->view('frontend/news_info', $data, true);
        $this->load->view('frontend/index', $data);
