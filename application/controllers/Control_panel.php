...
class Control_panel extends Base_Controller
{
    ...
    {
        if ($member_id != null) {
            $total = $this->back_model->count_user_by_parent($member_id);
            $perPage = 20;
            $this->pagination(base_url('control_panel/membersByMemberID/'), $perPage, $total);
            $data['users'] = $this->back_model->get_user_list_by_parent_ref_id($perPage, $this->uri->segment(4), $member_id);
            $data['menu'] = $this->load->view('backend/menu', '', true);
            $data['main_content'] = $this->load->view('backend/member/members', $data, true);
            $this->load->view('backend/index', $data);
        } else
