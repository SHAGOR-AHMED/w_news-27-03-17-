...
//    }
    ...
    {
        $inPoint = $this->db->select('SUM(point_value) as point')->from('tbl_user_point')->where('io_status', POINT_IN)->where('active', 1)->where('user_id', $user_id)->get()->row();
    ...
    private function getUserWithdrawalPoint($user_id)
    {
        $this->load->model('common_model');
        $ignore = array($this->common_model->getIncomingPointTypeIdByKey(Point_Purchased));
    ...
    public function userPointWithdraw($user_id, $balance, $point_exchange)
    {
        ...
        foreach ($inPoint as $item) {
            ...
            if (count($getTotalPoint) != 0) {
                $point_type_point = $this->db->select_sum('point_value')->from('tbl_user_point')->where('user_id', $user_id)->where('incoming_point_type_id', $item->incoming_point_type_id)->get()->row();
                $total_point = $item->point + $point_type_point->point_value;
